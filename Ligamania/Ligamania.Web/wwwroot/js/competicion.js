...
$(document).ready(function () {
    var table;
    var listcategorias;
    $.getJSON('getListaCategorias?idCompeticion=-1', function (categorias) {
        listcategorias = categorias;
    });
    $.getJSON('getListaCompeticiones', function (competiciones) {
        table = $('#tbCompeticiones').DataTable({
            "data": competiciones,
            "columns": [
                {
    ...
    $('#tbCompeticiones tbody').on('click', 'td.details-control', function () {
        ...
        if (row.child.isShown()) {
            ...
            // Open this row
            getDetails(row.data(), function (details, competicion) {
                var tabla = getTabla(details, listcategorias, competicion);
...
function getDetails(competicion, callbackDetails) {
    getTablaDetalle(competicion, function (tabla) {
        callbackDetails(tabla, competicion.id);
    });
};
function getTablaDetalle(competicion, callbackCategorias) {
    var urlString = 'getListaCategorias?idCompeticion=' + competicion.id;
    return $.ajax({
        type: 'GET',
        url: urlString,
        dataType: 'json',
        success: function (categorias) {
            callbackCategorias(categorias);
        },
