...
namespace VoleyPlaya.Domain.Models
{
    ...
    public class Edicion : IDomainDto
    {
        ...
        private async Task<bool> GenerarFaseFinalCircuito(TablaCalendarioCircuito calendario, DateTime fechaFaseFinal, int intervaloMin)
        {
            ...
            var numEquipos = numGrupos == 34 ? 170 : numGrupos == 28 ? 140 : numGrupos == 14 ? 70 : numGrupos == 6 ? 34 : 
                             numGrupos == 5 ? 25 :   numGrupos == 4 ? 12 :   numGrupos == 3 ? 10 :  numGrupos == 2 ? 6 : numGrupos == 1 ? 4 : 0;
            if (numEquipos == 0)
                return false;
            EdicionGrupo grupo;
            ...
            foreach (var par in partidosCalendario)
            {
                ...
                if (asignadoE2)
                {
                    var e2 = Equipos.Where(e => e.Id.Equals(equipo2)).FirstOrDefault();
                    var existeE2 = grupo.Equipos.FirstOrDefault(e => e.Id.Equals(equipo2));
                    visitante = e2.Nombre;
                    if (existeE2 == null)
                ...
                else
                {
                    ...
                    partido.NombreVisitante = visitante;
                }
        ...
        private int GetEquipo(string equipo, EdicionGrupo grupoFF, out bool asignado)
        {
            asignado = false;
        ...
        private int GetEquipoPerdedor(Partido partido, string equipo, out bool asignado)
        {
            asignado = false;
        ...
        private int GetEquipoGanador(Partido partido, string equipo, out bool asignado)
        {
            asignado = false;
            if (partido.Resultado.Local > partido.Resultado.Visitante)
            {
