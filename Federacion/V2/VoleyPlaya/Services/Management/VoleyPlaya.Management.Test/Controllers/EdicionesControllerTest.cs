...
public class EdicionesControllerTest
{
    private readonly EdicionesController _controller;

    public EdicionesControllerTest()
    {
        ...
        _services.AddScoped(x => unitOfWork.Object.EdicionRepository);
        ...
        _controller = new EdicionesController(mediator ?? throw new ArgumentNullException("Mandatory parameter", nameof(mediator)));
    }
    [Fact]
    public async void GetEdiciones_ResultOkObject()
    {
        ...
        var model = okObjectResult.Value as IEnumerable<EdicionDto>;
        Assert.NotNull(model);
        Assert.NotEqual(0d, model.Count(), 0);
    }
    [Fact]
    public async void GetEdicion_ResultNotFound()
    {
    ...
    [Fact]
    public async void AddEdicion_ResultOkObject()
    {
        var name = "New categoria";
        var actionResult = await _controller.Post(new AddEdicionCommand() { Nombre = name });
        var createdAtActionResult = actionResult.Result as CreatedAtActionResult;
        Assert.NotNull(createdAtActionResult);

        var model = createdAtActionResult.Value as EdicionDto;
        Assert.NotNull(model);
